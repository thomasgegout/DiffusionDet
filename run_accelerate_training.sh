...
# Toggle mixed precision (1=enable bf16, 0=disable)
USE_BF16=0
...
if [ "$NUM_GPUS" -gt 0 ]; then
    ...
    # Optionally disable bf16: when USE_BF16=0 we create a temporary copy of the
    # DeepSpeed config with bf16.disabled and omit the --bf16 flag so both
    # Accelerate/DeepSpeed and the training script agree.
    if [ "$USE_BF16" -eq 0 ]; then
        echo "Disabling bf16: creating temporary DeepSpeed config without bf16"
        TMP_DS_CONFIG="/tmp/deepspeed_config_no_bf16_$$.json"
        python3 - <<PY
import json
fn = "$DEEPSPEED_CONFIG"
with open(fn,'r') as f:
    cfg = json.load(f)
cfg.setdefault('bf16', {})['enabled'] = False
...
PY
    ...
    echo "Launching with Accelerate+DeepSpeed (num_processes=$NUM_GPUS, bf16=$USE_BF16)"
    accelerate launch \
